        ...
        if (Tools::getValue('updateOption')) {
            ...
            if (!empty($deliveryOption)) {
                Db::getInstance()->execute(
                    'INSERT INTO `'._DB_PREFIX_.bqSQL(MyParcelDeliveryOption::$definition['table']).'` (`id_cart`, `myparcel_delivery_option`)
                     VALUES ('.(int) $cart->id.', \''.pSQL(Tools::jsonEncode($deliveryOption)).'\')
                         ON DUPLICATE KEY UPDATE `myparcel_delivery_option` = \''.pSQL(Tools::jsonEncode($deliveryOption)).'\''
                );
                ...
                $return = array_merge(
                    array(
                        ...
                        'HOOK_PAYMENT'       => $this->getPaymentMethods(),
                        'carrier_data'       => $this->getCarrierList(),
