...
<div class="info-order box">
	...
	{foreach name=shipmentLoop from=$shipments item=shipment}
		...
		{if isset($shipment['tracktrace']) && $shipment['tracktrace']}
			...
			<a href="http://postnl.nl/tracktrace/?L={$languageIso|escape:'htmlall':'UTF-8'}&B={$shipment['tracktrace']}&P={$shipment['postcode']}&D=NL&T=C" target="_blank">
		...
		<table class="table table-bordered">
			...
			{foreach from=$shipment['history'] item=historyDetail}
				<tr class="item">
					...
					<td class="bold">
						<label for="cb_59">
							{$historyDetail['date_upd']|escape:'htmlall':'UTF-8'}
						</label>
					</td>
				</tr>
			{/foreach}
